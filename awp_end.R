...
awp <- read_csv("desktop/AWP/end/awp_organized_recode.csv")
...
f2a %>%
  ...
  summarize_all(.,sum,na.rm = TRUE)-> f2a2
...
f2a3 <- f2a2 %>%
  gather(variable,est,gender0:se.call_service3)

f3a3 <- f3 %>%
  gather(variable,se,se.gender0:se.call_service3)

f2a4 <- anti_join(f2a3,f3a3)
...
f4 <- left_join(f2a4,f3a3,by = 'id')

f4 <- f4 %>%
  select(-(c(id,variable.y)))

#write_csv(f4,"desktop/AWP/end/awp_accAll")
...
k2a %>%
  group_by(middleschool) %>%
  summarize_all(.,sum,na.rm = TRUE)-> k2a2
...
k2a3 <- k2a2 %>%
  gather(variable,pct,gender0:se.call_service3)

k3a3 <- k3 %>%
  gather(variable,pctse,se.gender0:se.call_service3)

k2a4 <- anti_join(k2a3,k3a3)
...
k4 <- left_join(k2a4,k3a3,by = 'id')

k5 <- left_join(f4,k4,by = 'id')

k5 <- k5 %>%
  select(-(c(id,variable.y.x,middleschool.y,variable.x.y,variable.y.y
  )))

#write_csv(k5,"desktop/AWP/end/awp_accAllrecode.csv")
